...
#define Ndarray_memset(s, c, size)                                             \
    (cudaMemsetAsync(s, c, size, CUDA_CUR_STREAM))
...
static void _cudaHandleError(cublasStatus_t status, const char *file,
                             int line) {
...
template <> DEV_FUNC HOST_FUNC const char *_format_for_type(const char &) {
    return "%c";
}
template <>
...
template <> DEV_FUNC HOST_FUNC const char *_format_for_type(const short &) {
    return "%hi";
}
template <>
...
template <> DEV_FUNC HOST_FUNC const char *_format_for_type(const int &) {
    return "%i";
}
template <>
...
template <> DEV_FUNC HOST_FUNC const char *_format_for_type(const long &) {
    return "%li";
}
template <>
...
template <> DEV_FUNC HOST_FUNC const char *_format_for_type(const long long &) {
    return "%lli";
}
template <>
...
template <> DEV_FUNC HOST_FUNC const char *_format_for_type(const float &) {
    return "%f";
}
template <> DEV_FUNC HOST_FUNC const char *_format_for_type(const double &) {
    return "%f";
}
template <>
...
void atomic_prob_add(float *a, float b) {
    ...
    do {
        assumed = old;
        old = elem_atomic_cas(addr, assumed,
                              float_as_int(prob_add(int_as_float(old), b)));
    } while (old != assumed);
}
...
void next_frame(bool fwd, unsigned num_edges, unsigned num_emissions,
                unsigned *sequence_idxs, unsigned *from_buffer,
                unsigned *to_buffer, float *weight_buffer,
                unsigned *emission_idxs, float *prev_frame, float *next_frame,
                float *am_scores, float *edge_buffer) {
    unsigned idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= num_edges) {
        return;
    }
    ...
    float val = prev_val + edge_weight +
                am_scores[sequence_idx * num_emissions + emission_idx];
...
DEF_KERNEL
void normalize(float *buffer, unsigned *sequence_idxs, unsigned num_edges,
               unsigned num_seqs, float *sum_output) {
...
void compute_result(float *edge_buffer, float *out, unsigned *emission_idxs,
                    unsigned *sequence_idxs, unsigned frame_stride,
                    unsigned seq_stride, unsigned num_frames, unsigned num_seqs,
                    unsigned num_edges) {
    unsigned idx = blockIdx.x * blockDim.x + threadIdx.x;
    ...
    atomic_prob_add(out + frame * frame_stride + seq_idx * seq_stride +
                        emission_idx,
                    score);
}
    ...
    for (unsigned seq = 0u; seq < n_seqs; seq++) {
        ...
        for (unsigned t = 0u; t < n_frames; t++) {
            ...
            for (unsigned e = 0u; e < n_emissions; e++) {
                const float val =
                    buffer[t * n_seqs * n_emissions + seq * n_emissions + e];
                if (val <= pruning) {
                    out << t << ' ' << e << ' ' << val << '\n';
                }
            }
...
std::vector<torch::Tensor> fbw_cuda(std::vector<torch::Tensor> torch_inputs) {
    ...
    torch::Tensor sum_output =
        torch::zeros({am_scores->size(0), am_scores->size(1)}, options);
    ...
    assert_cmp(Ndarray_DIMS(am_scores)[1], ==,
               Ndarray_DIMS(start_end_states)[1]);
    ...
    unsigned *d_from = reinterpret_cast<unsigned *>(
        Ndarray_DEV_DATA_int32(edges) + 0 * Ndarray_STRIDE(edges, 0));
    unsigned *d_to = reinterpret_cast<unsigned *>(
        Ndarray_DEV_DATA_int32(edges) + 1 * Ndarray_STRIDE(edges, 0));
    unsigned *d_emission_idxs = reinterpret_cast<unsigned *>(
        Ndarray_DEV_DATA_int32(edges) + 2 * Ndarray_STRIDE(edges, 0));
    unsigned *d_sequence_idxs = reinterpret_cast<unsigned *>(
        Ndarray_DEV_DATA_int32(edges) + 3 * Ndarray_STRIDE(edges, 0));
    ...
    float *d_edge_buffer = reinterpret_cast<float *>(
        device_malloc(n_edges * n_frames * sizeof(float)));
    if (!d_edge_buffer) {
        HANDLE_LAST_ERROR();
        abort();
    ...
    start_dev_kernel2(fill_array, n_fill_blocks, n_threads, 0,
                      (d_state_buffer_prev,
                       std::numeric_limits<float>::infinity(), n_states));
    HANDLE_LAST_ERROR();
    start_dev_kernel2(set_start_states, 1, n_seqs, 0,
                      (d_state_buffer_prev, d_start_states));
    HANDLE_LAST_ERROR();
    ...
    if (dump_alignment && batch_idx % dump_every == 0) {
        d_state_buffer_all = reinterpret_cast<float *>(
            device_malloc(n_states * (n_frames + 1u) * sizeof(float)));
        if (!d_state_buffer_all) {
            HANDLE_LAST_ERROR();
            abort();
        } // error should have been set in device_malloc
        Ndarray_memcpy(d_state_buffer_all, d_state_buffer_prev,
    ...
    for (unsigned t = 0u; t < n_frames; t++) {
        ...
        start_dev_kernel2(next_frame, n_blocks, n_threads, 0,
                          (true, n_edges, sequence_stride, d_sequence_idxs,
        ...
        HANDLE_LAST_ERROR();
        if (dump_alignment && batch_idx % dump_every == 0) {
            Ndarray_memcpy(d_state_buffer_all + (t + 1u) * n_states,
                           d_state_buffer_next, n_states * sizeof(float));
            HANDLE_LAST_ERROR();
        }
    ...
    for (unsigned t = n_frames; t > 0; t--) {
        ...
        HANDLE_LAST_ERROR();
        start_dev_kernel2(next_frame, n_blocks, n_threads, 0,
                          (false, n_edges, sequence_stride, d_sequence_idxs,
    ...
    start_dev_kernel2(
        normalize, n_frames, 1, n_seqs * sizeof(float),
    ...
    n_fill_blocks =
        (n_frames * n_seqs * n_emissions + n_threads - 1u) / n_threads;
    start_dev_kernel2(fill_array, n_fill_blocks, n_threads, 0,
                      (d_out, std::numeric_limits<float>::infinity(),
                       n_frames * n_seqs * n_emissions));
    HANDLE_LAST_ERROR();
    ...
    start_dev_kernel2(compute_result, n_blocks, n_threads, 0,
                      (d_edge_buffer, d_out, d_emission_idxs, d_sequence_idxs,
                       frame_stride, sequence_stride, n_frames, n_seqs,
                       n_edges));
    HANDLE_LAST_ERROR();
