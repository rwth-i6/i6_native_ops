...
import i6_native_ops.fast_viterbi.fast_viterbi_core as viterbi
...
def main():
    ...
    output, scores = viterbi.fast_viterbi(
        log_probs, edges, weights, start_end_states, seq_lens, 2,
    )
    best_sequence = list(output[:,0])
    score = float(scores[0])

    assert best_sequence == [0, 0, 1, 1, 1], f"Test failed: the output {best_sequence} is not as expected!"
    print(f"Test passed with Viterbi {score=:.2f}.")
